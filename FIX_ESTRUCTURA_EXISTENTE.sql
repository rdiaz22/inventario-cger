-- =====================================================
-- SOLUCIÓN: CORREGIR ESTRUCTURA EXISTENTE
...
-- 2. Agregar columnas faltantes a system_users
ALTER TABLE system_users ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE;
...
ALTER TABLE system_users ADD COLUMN IF NOT EXISTS position VARCHAR(100);
ALTER TABLE system_users ADD COLUMN IF NOT EXISTS role_id UUID;
ALTER TABLE system_users ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT true;
ALTER TABLE system_users ADD COLUMN IF NOT EXISTS last_login TIMESTAMP WITH TIME ZONE;
ALTER TABLE system_users ADD COLUMN IF NOT EXISTS created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
...
-- 8. Insertar usuario por defecto solo si no existe
...
-- 9. Insertar log por defecto solo si no existe
